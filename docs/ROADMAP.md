...
- Automated regression test suite with timed component injection — headless single-sketch execution itself is done (`vemcode <sketch.cpp>` — see `main.cpp`'s `run_headless`, no `QT_QPA_PLATFORM=offscreen` needed since it uses `QCoreApplication` and never touches a display), but it currently has zero injection surface: it can only test sketches that run autonomously with no external stimulus. Design (not yet built):
  - **CLI**: `vemcode Test.cpp [Test.timeline] [key=value ...]` — `argv[1]` sketch path (unchanged); any following bare arg (no `=`) is the optional timeline file path (error if more than one given); any `key=value` arg is an option — `timeout=N` (hard wall-clock ceiling in real seconds, independent of `speed`, default unlimited/Ctrl+C like today) and `speed=N` (sketch-time multiplier via the existing `set_speed_multiplier()`, default 1 = real-time, no auto speed-up). Unknown key = hard error, to catch typos rather than silently no-op
  - **`<Sketch>.timeline` file** — sidecar file next to the sketch, one event per line: `time, VERB, target, [args...]` (one consistent column shape for every line — `VERB` is either `ASSERT` for checks or an action verb for stimulus, e.g. `5, ASSERT, PIN, 12, HIGH` / `10, PRESS, BUTTON1` / `15, SET, POT1, 512`)
  - **Component resolution** — `target` names are resolved against `CircuitDetector`'s already-detected `pin_name` (the literal `#define` token, e.g. `BUTTON1`, or the shared prefix for multi-pin components, e.g. `MOTOR1` from `MOTOR1_ENA`/`MOTOR1_IN1`) — no new naming scheme needed, this data already exists from the "Components detected" list `run_headless` already prints
  - **Action verbs, input-capable components only** (output-only components like LED/servo/buzzer/RGB LED/seven-segment/HBridge/Stepper/LCD have nothing to inject — they're only ever `ASSERT`ed on): `PRESS`/`RELEASE`/`SET` for button/switch (`inject_pin`/`inject_button_bounce`), `SET` for potentiometer/analog sensor/generic input (`inject_analog`), `SET` for distance/IR sensor (`inject_pulse_duration`), `MOVE`/`PRESS` for joystick (axis `inject_analog` + button `inject_pin`), `SET_COLOR` for color sensor (`inject_color`, blocked on the still-open color-sensor Array-grouping detection bug above), `SEND` for Serial/SoftwareSerial (`inject_serial`/`runtime().inject_soft_serial`), `SET_RESPONSE` for Wire/SPI virtual devices (`inject_wire_device`/`inject_spi_bytes`, targeted by a fixed `WIRE`/`SPI` keyword rather than a detected component name since these aren't auto-detected from source). Rotary encoder (`ROTATE`) needs a simplified best-effort quadrature pulse burst rather than real-time-spread phase transitions
  - **`ASSERT` verbs** — `ASSERT, PIN, <name-or-number>, <HIGH|LOW|value>` against a pin-state map fed by the already-existing `on_pin_changed` callback (currently wired for GUI mode only); `ASSERT, SERIAL_CONTAINS, "text"` against an accumulated Serial output buffer. `ASSERT, VAR, <name>, <value>` (watched-variable checks) is a further-out addition — variable watch is currently only wired through `SketchThread`'s `setWatchList`/`variableChanged`, not present in headless mode at all
  - **Scheduler** — headless mode is single-threaded (unlike the GUI's separate `SketchThread`), so firing due timeline events can be inline in the existing `run_loop()` loop with no new locking; on completion (timeout reached, all events fired + sketch exits, or Ctrl+C) print a pass/fail summary and exit with a nonzero code if any `ASSERT` failed — the actual mechanism that makes this usable as a real CI-style test rather than "run it and read the output yourself"
