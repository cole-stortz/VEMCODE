...
- [ ] `CircuitDetector` refactored to loop over the registry — detection runs in three confidence tiers: (1) `detect_pattern` source patterns first, (2) `detect_multi` pin-role matching, (3) `detect_single` keyword matching as final fallback; a match at a higher tier short-circuits the lower tiers; no component-specific knowledge remains in `CircuitDetector` itself
  ...
  - [ ] `detect_pattern` tier — Servo `.attach(`, `pulseIn(` for distance/color routed through each component's registered `detect_pattern` list instead of bespoke regex blocks
  - [ ] LCD's `LiquidCrystal lcd(RS, E, D4, D5, D6, D7)` ctor-arg fallback generalized into a reusable "extract N args from a `ClassName var(...)` call" extractor, not an LCD-specific regex
