...
- In-GUI timeline editing + running — `.timeline` fixtures currently only run headlessly (`timeline=true` CLI flag), printing PASS/FAIL to the console with no visual feedback; add a way to open/edit a sketch's `.timeline` file and run it from the GUI while watching the canvas, serial monitor, and signal timeline update live as each action/ASSERT fires, rather than only getting a text summary after the fact. Editing could start as just opening the `.timeline` file in the existing code editor (it's a plain line-based format already) before building a dedicated structured table editor (time/verb/target/args columns) with add/remove-row UX. Hardest part: `TestRunner` (`src/core/host/timeline.h`) was built around headless's single-shot `run_headless` loop, which drives `fireDueEvents(sketchSeconds)` off its own clock; wiring the same class into `SketchThread`'s continuously-running GUI loop needs an equivalent "sketch-time seconds" clock there, plus a place for live PASS/FAIL/action output to show (likely a new "Timeline" tab in the debug panel, mirroring the headless console output)
- Record-and-replay timeline capture — pairs with the in-GUI timeline runner above: a Record toggle captures live canvas interactions (button presses, potentiometer drags, etc.) and the resulting pin/serial output into a new `.timeline` fixture automatically, reusing the same `TestRunner`/signal-timeline plumbing instead of hand-authoring fixtures line by line
