...
void CircuitDetector::detect_custom_components(
    const std::string& source,
    const std::map<std::string, std::string>& defines,
    const std::map<std::string, std::vector<int>>& arrays,
    std::set<int>& claimed)
{
    for (const auto& def : ComponentRegistry::instance().all()) {
        if (def.detect_custom) def.detect_custom(*this, source, defines, arrays, claimed);
    }
}

void CircuitDetector::add_detected_component(const DetectedComponent& comp, std::set<int>& claimed) {
    components_.push_back(comp);
